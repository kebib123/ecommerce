...
////        Mail::to($user->email)->send(new OrderMail($data));
            ...
            if ($order && $save) {
                Cart::destroy();
                return response()->json(['message'=>'Order placed successfully','order_id' => $order_id, 'route' => route('checkout-payment', ['order_id' => $order_id])], 200);
...
//            dd($order);
            return view('frontend/pages/checkout/checkout-payment', compact('order', 'cartItem'));
        }
